...
from ultralytics.nn.smoke_modules import ECPConvBlock, SmokeMFFPN, EMA, RCM
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if m.i in embed:
                ...
                if m.i == max_idx:
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        head = self.model[-1]
        # SmokeMFFPN 같은 모듈이 리스트를 반환할 수 있음 - flatten 처리
        head_inputs = []
        for j in head.f:
            feat = y[j]
            if isinstance(feat, list):
                head_inputs.extend(feat)  # 리스트면 펼침
            else:
                head_inputs.append(feat)
        x = head(head_inputs, batch)  # head inference
        return x
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # ch[x]가 리스트인 경우 (예: SmokeMFFPN 출력) flatten 처리
            ch_list = []
            for x in f:
                if isinstance(ch[x], list):
                    ch_list.extend(ch[x])  # 리스트면 펼침
                else:
                    ch_list.append(ch[x])  # 단일 값이면 추가
            args.insert(1, ch_list)
        elif m is SmokeMFFPN:
            hidden_dim = args[0]     # YAML에서 적은 256 (hidden_dim)
            c1 = [ch[x] for x in f]  # 이전 레이어들의 채널 리스트 [S3, S4, F5]
            args = [c1, hidden_dim]
            # SmokeMFFPN은 [C1, C2, C3] 리스트를 반환하며, 모두 hidden_dim 채널
            c2 = [hidden_dim, hidden_dim, hidden_dim]
        elif m is CBLinear:
            c2 = args[0]
            c1 = ch[f]
            args = [c1, c2, *args[1:]]
